  ...
  image: "assets/images/roosevelt.gif"
      ...
      var game = {
        ...
        beginQuestion: function() {
          ...
          $("#twowrapper").html("<h2> TIME REMAINING <span id='counter '>30</span> Seconds</h2>")
          ...
          for (var i = 0; i < gameQuestions[game.questionNumber].answers.length; i++) {
            $("#twowrapper").append('<button class="answerButton"id ="button-'+i+'" data-name"'+gameQuestions[game.questionNumber].answers[i]
      ...
      results: function(){
        clearInterval(timer);
        $("#twowrapper").html("<h2>All Done!</h2>");
      ...
      clicked: function(e){
        clearInterval(timer);
        if ($(e.target).data("name")===gameQuestions[game.questionNumber].correctAnswer)
        {
